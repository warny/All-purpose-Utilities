...
**✅ Corrigé (2026-07-17, complété 2026-07-18).** 15 tests dans `UtilsTest/Data/SqlPrettyPrinterTests.cs`, plus 3 tests d'intégration dans `UtilsParserSqlQueryParserTests.cs`.
...
- Le mode `Prefixed` : virgule en début de ligne pour SELECT (2 et 3 colonnes), FROM/WHERE, GROUP BY
  (avec 2 items), ORDER BY (avec 2 items), JOIN, UPDATE/SET/WHERE, IndentSize=2
- Le mode `Suffixed` : virgule en fin de ligne pour SELECT (2 et 3 colonnes), FROM/WHERE, GROUP BY
  (avec 2 items), JOIN
- Tous les tests utilisent `SqlQueryAnalyzer.Parse(sql)` directement ; aucune construction
  programmatique de segment.

Note : l'extraction de FROM/WHERE/GROUP BY etc. nécessitait PR #468 (`QuantifierNode` +
`ParseTreeNavigator` deux passes) et la correction des helpers de `UtilsParserSqlQueryParser`
(`GetOptionalChild`, `GetChildren`, `isDistinct`, `isRecursive`) pour traverser les wrappers
`QuantifierNode`. Ces corrections font partie de la PR #469.

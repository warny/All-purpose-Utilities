...
foreach($package in $manifest.packages){
    ...
    if($p.Version-ne$manifest.version-or$p.PackageVersion-ne$manifest.version){throw "$($package.packageId) evaluates divergent Version/PackageVersion values."}
...
Write-Host "Validate: all $($manifest.packages.Count) projects evaluate to synchronized version $($manifest.version)."

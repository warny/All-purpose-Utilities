...
$mapping = [ordered]@{
    ...
    'Ubuntu packaged validation' = @('Package validation (ubuntu-latest)')
    'Windows packaged validation' = @('Package validation (windows-latest)')
    'Source gates' = @('source-gates')
    'Candidate assembly' = @('assemble-pr-candidate')
}

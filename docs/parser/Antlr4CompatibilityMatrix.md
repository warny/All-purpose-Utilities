...
|---|---|---|
| Rule parameters (`rule[int x]`) | Parsed with balanced-text preservation and stored as raw metadata text; each declaration split by top-level comma, name extracted lexically (e.g. `value` for `int value`); raw declarations preserved. Generated C# execution contexts expose `GetRuleParameter`, `TryGetRuleParameter`, and `GetRuleParameterDescriptors` helpers. | No argument passing, typed binding, auto-binding, or generated parser signature changes; frames are not auto-populated; `$param` and rule call arguments (`callee[42]`) are not evaluated or supported. |

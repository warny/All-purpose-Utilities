...
| Grammar actions and `@members` | Preserved as metadata where visible, but not injected or executed. | Not supported; classified/skipped when visible to runtime discovery. | Not supported; visible constructs emit generator warning `UP1029`. `@members` users should provide callable members through a separate partial class. | Represented-only; no member-injection model. |

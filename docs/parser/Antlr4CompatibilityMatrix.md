...
|---|---|---|---|---|---|---|
| Semantic predicates (`{ condition }?`) | Yes | Yes (predicate metadata) | Partially (policy-routed) | Policy-dependent only | Policy-routed: conservative by default (`NotEvaluated`), with optional expression-backed evaluator when explicitly configured | `SemanticPredicateNotEnforced` (`UP1006`) when evaluator returns `NotEvaluated` without detailed metadata; `UP1026` when an expression-backed evaluator returns detailed compile/adaptation/compiled-execution failure metadata. `UP1028` is reserved for explicit execution-disabled policies and is not used by current expression-backed adapters |

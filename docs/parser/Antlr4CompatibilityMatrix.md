...
The assignment-label syntax `$c.value`/`$x.value` rewrites to required helper access only in inline parser actions and `@after`; `$xs.value`, `$child.value`, and `$rule.value` remain unsupported and should produce deterministic transformer diagnostics rather than rewrites. Generated `Parse(...)` stays conservative, and `ParserEngine` does not gain target-language-specific C# logic.

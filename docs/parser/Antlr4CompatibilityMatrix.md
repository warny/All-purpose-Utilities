...
|---|---|---|---|
| Lexer action `$text`, `$type`, `$channel`, `$mode` reads | Preserved as raw target code | Unsupported | Rewritten to generated execution-context helpers. `$text` reads `LexerActionExecutionContext.Text` through `GetRequiredLexerText(context)` and exposes context-level accepted token/chunk text. |
| Lexer action `$text = ...`, `$text ??= ...`, `$type = ...`, `$channel = ...` writes | Preserved as raw target code | Unsupported | Deterministic transformer error |

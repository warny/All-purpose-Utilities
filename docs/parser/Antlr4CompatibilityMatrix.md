...
| Lexer action `$type = ...` and `$channel = ...` simple writes | Preserved as raw target code | Unsupported | Rewritten to bounded action-result helpers; applied before lexer commands, so commands remain authoritative |
| Other lexer action writes such as `$text = ...`, `$text ??= ...`, `$mode = ...`, `$line = ...`, `$pos = ...`, compound writes, and expression writes | Preserved as raw target code | Unsupported | Deterministic transformer error |

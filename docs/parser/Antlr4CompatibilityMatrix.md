...
| Lexer predicates/actions | Not executed by conservative `Parse(...)`. | Not supported. | Simple lexer inline actions and simple lexer predicates are supported only through generated-C# opt-in. Predicates are evaluated during lexer matching and reject only the current path; actions execute after token acceptance and before accepted lexer commands are applied. Tests cover rule references/fragments, simple quantifiers, duplicate hook positions, false-predicate/action ordering, `skip`, `channel(...)`, `type(...)`, `more`, `mode(...)`, and `pushMode(...)`/`popMode` mode transitions. | Lexer `$...` support is limited to generated-C# opt-in inline action reads (`$text`, `$type`, `$channel`, `$mode`) through the optional C# transformer; lexer predicate attributes, other writes, runtime-inline lexer execution, a separate lexer runtime, public quantifier iteration indexes, rollback of external side effects, and complete ANTLR modes/channels/commands compatibility remain unsupported. |
...
| Lexer action `$type = identifierOrString;` and `$channel = identifierOrString;` simple writes | Preserved as raw target code | Unsupported | Rewritten to bounded `LexerActionExecutionResult` helper writes; generated hooks do not mutate `Token` directly; `LexerEngine` applies accepted writes before lexer commands, so `type(...)`, `channel(...)`, `skip`, and `more` remain authoritative |
| Other lexer action writes such as `$text = ...`, `$mode = ...`, `$line = ...`, `$pos = ...`, `$type += ...`, `$channel ??= ...`, `$type++`, `$channel++`, complex expression writes, `ref`/`out`, and chained attributes | Preserved as raw target code | Unsupported | Deterministic transformer error |

...
| Rule returns (`returns [int value]`) | Recognized, preserved as raw `Rule.Returns` metadata, exposed by passive descriptors, and emitted as explicit compatibility diagnostic `RuleReturnsIgnored` (`UP1007`). Generated C# execution contexts expose explicit lifecycle helper methods to read/write `context.InvocationFrame` returns and inspect descriptors. | Not typed, auto-allocated, or propagated to caller frames; passive invocation frames do not populate return stores automatically; no typed return fields/properties, implicit return variables, `$rule.value`, labeled rule-reference return access, or return propagation are added. |

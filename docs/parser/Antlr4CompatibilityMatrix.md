...
Static binding diagnostics validate local parser-rule targets and direct/transitive imported parser-rule targets only when the imported grammars are same-project `.g4` `AdditionalFiles`, their declared grammar names resolve uniquely, and the rule target is not ambiguous. Local declarations take priority. Missing imports, duplicate grammar declarations, ambiguous imports, aliased imports, lexer-only rules, and unresolved targets remain metadata-only. The syntax is not extended, no AST merge or C# expression evaluation is performed, imports are not made executable in a new way, and `Parse(...)` remains metadata-only.

...
| Inline actions (`{ code }`) | Yes | Yes (embedded action metadata) | No target-language semantic resolution | No by default | Parsed-but-not-executed compatibility only | `InlineActionStoredNotExecuted` (`UP1005`) by default when action execution outcome is `NotExecuted` without detailed metadata; custom `ParserActionExecutionOutcome.NotExecuted(diagnostic, ...)` may surface detailed embedded-code diagnostics (for example `UP1028`) without duplicating `UP1005` |

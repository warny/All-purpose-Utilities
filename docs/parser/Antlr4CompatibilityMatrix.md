...
The generated-C# path supports grammar-level `@lexer::header`, `@lexer::members`, and `@lexer::footer` in the same limited compatibility model as parser named actions. Lexer headers are emitted after parser headers and before generated types, lexer members are emitted into the generated execution context after parser members, and lexer footers are emitted after parser footers with dedicated lexer markers. This does not support lexer inline actions, lexer predicates, `$...` rewriting, a separate generated ANTLR lexer type, or full lexer target-language compatibility.

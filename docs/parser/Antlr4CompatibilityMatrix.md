...
| Rule-call argument clause (`callee[...]`) | Recognized and preserved as raw text on `RuleRef.RawArguments` (outer brackets excluded). Both the runtime ANTLR converter and the source-generator G4 parser preserve the raw text. Reported with `UP1037 RuleCallArgumentsPreservedAsMetadata`. | Raw argument text is not evaluated, not parsed as C# expressions, and not bound to child rule parameters. `PendingChildSeeds`, `InvocationFrame.Parameters`, and frame behavior are unchanged. Generated `Parse(...)` and rule signatures are unchanged. Use `SetNextRuleParameter(...)` for explicit parameter seeding. `$param` is not supported. |

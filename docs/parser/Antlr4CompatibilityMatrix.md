...
| Rule locals (`locals [int x]`) | Recognized, preserved as raw `Rule.Locals` metadata, exposed by passive descriptors, and emitted as explicit compatibility diagnostic `RuleLocalsIgnored` (`UP1008`). | Not executed, typed, or allocated from grammar metadata; passive invocation frames do not populate local stores automatically. |

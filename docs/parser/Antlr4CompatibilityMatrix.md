...
| Lexer predicates/actions | Not executed by conservative `Parse(...)`. | Not supported. | Simple lexer inline actions and simple lexer predicates are supported only through generated-C# opt-in. Predicates are evaluated during lexer matching; actions execute after token acceptance. | Lexer `$...`, runtime-inline lexer execution, a separate lexer runtime, rollback of external side effects, and complete modes/channels/commands compatibility remain unsupported. |

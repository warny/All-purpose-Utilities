...
The generated-C# positional binding opt-in now includes bounded generator-time diagnostics for certain failures only when the call target is the caller grammar's unique local parser rule. Imported rules are not statically concluded by `APU0107` because generated definitions currently contain only local rules and cannot execute imported targets through `ParserDefinition.Imports`. Missing imports or rules, aliased imports, duplicate declared grammar names, ambiguous imported declarations, explicit mapping helpers, and manually installed runtime policies are not statically concluded by this pass. This does not change the runtime contract: arguments remain positional only, simple literals only, exact arity only, allowlisted declared types only, no default-value consumption, no named arguments, no expressions, no generated ANTLR-style rule signatures, and no change to `Parse(...)`. A grammar file with certain local generated-binding errors is not emitted, but other valid grammar files continue to generate.
...
With `UtilsParserEnableGeneratedRuleArgumentBinding=true`, this reports `APU0107` because `1 + 2` is not a supported simple literal. Imported parser-rule targets do not produce `APU0107` in the current generator because the emitted `ParserDefinition` for the caller does not include imported rules. The same-project resolver remains preparatory infrastructure for a future composition plan, but diagnostics must not conclude on a target that the generated parser cannot resolve. With the option absent or `false`, the argument remains metadata only and no generated-binding diagnostic is produced.

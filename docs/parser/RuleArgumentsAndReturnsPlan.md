...
The generated-C# positional binding opt-in now includes bounded generator-time diagnostics for certain failures when the call target is either the caller grammar's unique local parser rule or a unique parser rule resolved through direct or transitive same-project `.g4` `AdditionalFiles` imports. Local parser rules retain priority over imported rules. Missing imports or rules, aliased imports, duplicate declared grammar names, ambiguous imported declarations, explicit mapping helpers, and manually installed runtime policies are not statically concluded by this pass. This does not change the runtime contract: arguments remain positional only, simple literals only, exact arity only, allowlisted declared types only, no default-value consumption, no named arguments, no expressions, no generated ANTLR-style rule signatures, and no change to `Parse(...)`. A grammar file with certain generated-binding errors is not emitted, but other valid grammar files continue to generate.

The generator pipeline parses each `.g4` file independently from its path, source text, and own `AdditionalFiles` metadata such as `Namespace` and `ClassName`. Unchanged parsed-file values are reusable across global option changes and unrelated file edits. The parsed project is still aggregated with `Collect()` so declared-grammar-name indexing, direct/transitive import resolution, ambiguity checks, validation, and emission can rerun project-wide when one grammar file or the binding option changes; no import-subgraph-only recalculation guarantee is made.
...
With `UtilsParserEnableGeneratedRuleArgumentBinding=true`, this reports `APU0107` because `1 + 2` is not a supported simple literal. The same diagnostic contract also applies when the target parser rule is reached through a uniquely resolved direct or transitive import among `.g4` files supplied as same-project `AdditionalFiles`. Resolution is intentionally conservative: it uses declared grammar names, lets local rules shadow imported rules, skips aliased imports because call sites do not represent alias qualification, and ignores missing imports/rules or ambiguous imported declarations without emitting `APU0107`. With the option absent or `false`, the argument remains metadata only and no generated-binding diagnostic is produced.

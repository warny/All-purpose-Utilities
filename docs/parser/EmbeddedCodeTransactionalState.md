...
Generated `$x.value` reads are projections over the current invocation frame's immutable assignment-label result store. They do not consult list-label storage or `LastCompletedChildCall`, so failed-alternative rollback and memoized child reuse observe only the current successful call-site label. `$rule.value` reads the current frame return store directly. Both operations are read-only, preserve present-null entries, and add no external side effects or new rollback authority. Runtime absence is reported by `ParserAttributeAccessException`; statically knowable invalid roots and return names are rejected by `UP0014` before source emission.

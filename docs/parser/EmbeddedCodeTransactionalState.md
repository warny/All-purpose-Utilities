...
Dictionary and set hashing is deterministic and independent from native enumeration order. Dictionary entries and set elements are sorted before hashing: comparable keys/elements are ordered by their comparable value, while non-comparable but explicitly hashable values are ordered by their structural parser-state hash, with only a final enumeration-index tie-breaker for indistinguishable sort keys. This avoids depending on `Dictionary<TKey,TValue>` enumeration order.

...
## Attribute reads and transactional state

Generated `$x.value` reads project one required return from the current invocation frame's immutable assignment-label result store; absence throws `ParserAttributeAccessException`. Generated `$xs.value` reads project `IReadOnlyList<object?>` from list-label storage: successful call order is preserved, present-null entries are included, missing return keys are skipped, and absence yields an empty list. Neither form consults `LastCompletedChildCall`, so failed-alternative rollback, repetition probes, and memoized child reuse observe only successful current call-site labels. `$rule.value` reads the current frame return store directly. All forms are read-only, add no external side effects or rollback authority, and statically knowable invalid roots, return names, writes, or ambiguous label kinds are rejected by `UP0014`.

...
These reads do not add parser-managed transactional state, mutable lexer action results, or rollback authority. They are passive metadata passed to the accepted lexer action before lexer commands such as `skip`, `type(...)`, `channel(...)`, `mode(...)`, `pushMode(...)`, or `popMode` apply to later token emission or tokenization. With `more`, each accepted action reads the current accepted chunk coordinates, not a final accumulated token coordinate. Lexer attribute writes, lexer predicate `$...` reads, runtime-inline lexer execution, and a separate lexer runtime remain unsupported.

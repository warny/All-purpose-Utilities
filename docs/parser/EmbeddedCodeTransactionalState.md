...
ANTLR-style convenience forms such as `$x.value`, `$xs.value`, `$rule.value`, `$param`, and `$local` are not parser transactional primitives. They are not core parser syntax and are not rewritten by default. With `NoOpParserEmbeddedCodeTransformer`, these forms remain unchanged target-language text. An optional C# ANTLR-style transformer may rewrite them to generated helper calls, but that transformer remains a compatibility layer outside parser core behavior.

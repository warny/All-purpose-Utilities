...
- [`ANTLRCompatibility.md`](./ANTLRCompatibility.md): Canonical ANTLR compatibility reference, including conservative defaults, explicit labeled parser-rule return/result semantics, repeated list-label target validation, embedded-code preservation by default, optional target-language transformers for `$...` convenience syntax including C# transformer-only current-rule local writes and logic-stage current-rule return writes, embedded-code opt-in paths, represented metadata, diagnostics, the separate explicitly opt-in untyped and typed positional/named literal call policies, including typed simple-literal defaults, their conversion rules, and their non-goals.
- [`Antlr4CompatibilityMatrix.md`](./Antlr4CompatibilityMatrix.md): Feature matrix for default, runtime-inline, and generated-C# behavior, including embedded-code preservation by default, optional ANTLR-style attribute rewriting plus local-write and logic-stage current-rule return-write rewriting through a target-language transformer, labeled assignment/list result access, partial opt-in untyped and typed positional/named literal call binding, invocation-frame metadata, and state-aware memoization constraints.

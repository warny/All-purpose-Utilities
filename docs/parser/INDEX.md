...
- [`ANTLRCompatibility.md`](./ANTLRCompatibility.md): Canonical ANTLR compatibility reference, including conservative defaults, explicit labeled parser-rule return/result semantics, the limited read-only generated-C# assignment `$x.value`, list `$xs.value`, and current-rule `$rule.value` rewrite, embedded-code opt-in paths, represented metadata, diagnostics, the separate explicitly opt-in untyped and typed positional/named literal call policies, including typed simple-literal defaults, their conversion rules, and their non-goals.

...
|---|---|---|
| Rule parameters `rule[int x]` | `Rule.Parameters` as raw text (each declaration split and name extracted lexically) | No ANTLR-compatible argument passing; generated C# lifecycle hooks can inspect parameter descriptors, read explicitly-supplied frame values, and seed parameter values for the next named child rule via `SetNextRuleParameter`; seeds are rollback-safe; `callee[expr]` is not evaluated; `$param` is not supported. |

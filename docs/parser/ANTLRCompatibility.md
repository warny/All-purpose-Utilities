...
Parser named-action support is parser-side only: `@header` and `@parser::header` emit parser header code, `@parser::members` emits members inside the generated parser execution context/class, and `@parser::footer` emits deterministic trailing parser source. Multiple supported parser named-action blocks are emitted in grammar source order. Lexer named actions (`@lexer::header`, `@lexer::members`, `@lexer::footer`) and arbitrary scopes such as `@tree::members` are unsupported and produce deterministic generator diagnostics instead of being injected into parser code. The default/no-op transformer preserves named-action content unchanged; optional transformer behavior remains opt-in, and `$...` current-rule attribute rewriting is intentionally limited to parser actions/lifecycle code, not parser header/member/footer content. Parser members can be called from generated inline parser actions and supported `@init`/`@after` lifecycle hooks.

...
The complete call is validated and parsed before any pending child seed is written. Successful values use the managed pending-child seed store, overwrite an existing seed for the same target rule and parameter, preserve unrelated seeds, and distinguish a present `null` value from an absent parameter. Rollback restores pending seeds. Generated state-aware memoization includes these pending values using deterministic hashing for exactly `null`, `bool`, `int`, `long`, `double`, `string`, and `char`, so calls such as `child[1]` and `child[2]` cannot share a completed result created under a different bound value. This support applies only to parser rules; it does not add lexer arguments, `$param`, `$x`, `$x.value`, `$rule.value`, return binding, or label-backed values.

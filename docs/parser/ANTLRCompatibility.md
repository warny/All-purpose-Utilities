...
- ANTLR-style `$...` convenience forms such as `$x.value`, `$xs.value`, `$rule.value`, `$param`, and `$local` are not core parser syntax; an optional C# embedded-code transformer may rewrite them, while the default no-op transformer leaves them unchanged;
...
## Limited generated-C# parser return attributes

The generated-C# opt-in path supports a deliberately narrow, read-only attribute rewrite in parser inline actions and parser `@after` blocks:

- `$x.value` reads return `value` from the successful assignment-labeled child call `x=child`;
- `$xs.value` projects return `value` from successful list-labeled child calls `xs+=child` in execution order;
- `$rule.value`, where `rule` is the exact current parser-rule name, reads return `value` from the current invocation frame.

Assignment and current-rule forms rewrite to generic required-access helpers and have expression type `object?`. A list-label form rewrites to `GetLabeledRuleCallReturns(...)` and has expression type `IReadOnlyList<object?>`; it preserves successful execution order, includes present-null entries, skips calls without the return, and returns an empty list when no successful result exists. When one list label targets multiple parser rules, validation retains every target and accepts a return declared by at least one target, matching the runtime projection that skips results without that key; validation is therefore independent of alternative order. No declared return type is used for conversion or an inserted cast; callers must cast or project explicitly. A statically knowable unknown root, token label, return undeclared by every list target, or assignment/list label-name ambiguity produces blocking generator diagnostic `UP0014`. At runtime, a rule-wide valid assignment label can still be absent on the selected alternative or before its call executes; required access then throws `ParserAttributeAccessException` with a deterministic message. Current-rule name matching has precedence over a same-named label, and all name matching is ordinal.

`$x.value` and `$xs.value` are rejected in `@init`, while `$rule.value` is allowed there and fails at runtime if the return has not been set. Attribute references in semantic predicates are rejected in this increment because predicate contexts do not expose the same managed frame access safely. Both assignment and list forms are read-only. Writes to parameters, returns, labels, list labels, token labels, and lexer attributes remain unsupported; only current-rule local writes are available through the optional C# transformer described below. `ref`/`out`, increment/decrement expression values, chains, bare labels such as `$x`/`$xs`, list indexing syntax such as `$xs[i]`, `$text`, `$ctx`, `$start`, `$stop`, token attributes, lexer attributes, and general ANTLR attribute expressions remain unsupported. Attribute-like text inside C# strings, character literals, and comments is not rewritten; interpolated strings are copied as opaque string syntax rather than parsing interpolation expressions.

The existing explicit result helpers and `SetRuleReturn(...)` remain the lower-level API. The rewrite does not add implicit variables, typed label properties, automatic propagation, or public generated parser method changes. `Parse(...)` remains conservative and does not execute embedded code.
...
> Lexer inline actions and predicates: simple source-generator C# lexer inline actions and simple lexer predicates are supported only through the explicit opt-in generated path. `Parse(...)` remains conservative. Predicates run during lexer matching and reject only the current path; actions run after token acceptance and before accepted lexer commands are applied. Commands from rejected paths are not applied, and actions are not executed when an earlier predicate rejects that path. The tested boundary includes rule references, fragments, simple quantifiers, duplicate hook positions, `skip`, `channel(...)`, `type(...)`, `more`, and mode transitions with `mode(...)` and `pushMode(...)`/`popMode`. `AlternativeIndex` and `ElementIndex` identify source hook locations, not quantified runtime iterations. Lexer `$...` rewriting is limited to generated-C# opt-in inline action reads (`$text`, `$type`, `$channel`, `$mode`) plus simple `$type = ...` and `$channel = ...` inline-action writes; lexer predicate attributes, other writes, runtime-inline lexer execution, a separate runtime lexer, complete ANTLR mode/channel/command semantics, generalized action buffering/replay, and external side-effect rollback remain unsupported.


### Limited generated-C# lexer `$...` attributes

Generated-C# opt-in lexer inline actions may use the optional C# ANTLR-style transformer for a deliberately small lexer attribute subset: `$text`, `$type`, `$channel`, `$mode`, `$line`, and `$pos` reads plus simple `$type = identifierOrString;` and `$channel = identifierOrString;` write statements. `EmbeddedLexerAttributeRewriter` rewrites these forms to generated helpers on the execution context. `$text` becomes `GetRequiredLexerText(context)` over passive `LexerActionExecutionContext.Text` and exposes the accepted token/chunk text available to the accepted lexer action context; tests lock this as context-level accepted text for fragment and lexer-rule-reference actions, not a guaranteed fragment-local or subrule-local slice. Simple tokens, quantified tokens, and alternatives read the accepted selected text. Actions paired with `skip`, `type(...)`, or `channel(...)` read text before the command suppresses, retags, or hides the token. With `more`, the action reads the current accepted chunk before final accumulation, and the later token action reads its own chunk text.

`$type` becomes `GetRequiredLexerType(context)` and reads `LexerActionExecutionContext.TokenType`; `type(...)` is applied after the current action, so the action sees the pre-command type. `$channel` becomes `GetRequiredLexerChannel(context)` and reads `LexerActionExecutionContext.Channel`; `channel(...)` is applied after the current action, so the action sees `DEFAULT_CHANNEL` unless earlier runtime state already provided another channel. `$mode` becomes `GetRequiredLexerMode(context)` and reads `LexerActionExecutionContext.Mode`, the mode that accepted the current token/chunk; `pushMode(...)`, `mode(...)`, and `popMode` affect subsequent tokenization after the current action. Fragment and lexer-rule-reference actions read the accepted outer token context metadata for these passive values in current tests. `$line` becomes `GetRequiredLexerLine(context)` and reads `LexerActionExecutionContext.Line`, the 1-based line at the beginning of the accepted token/chunk from `SourceSpan.Line`. `$pos` becomes `GetRequiredLexerPos(context)` and reads `LexerActionExecutionContext.Column`, the 1-based column at the beginning of the accepted token/chunk from `SourceSpan.Column`. In this runtime, `$pos` follows `SourceSpan.Column` and is therefore 1-based; it should not be inferred as full ANTLR `charPositionInLine` compatibility.

Only simple `$type = identifierOrString;` and `$channel = identifierOrString;` write statements are supported. The generated hook does not mutate `Token` directly. It sets `LexerActionExecutionResult.TokenType` or `LexerActionExecutionResult.Channel`; `LexerEngine` applies those bounded mutations to the accepted token after accepted lexer actions run and before lexer commands, so `type(...)`, `channel(...)`, `skip`, and `more` keep their existing command/chunking authority. Read helpers such as `$type` and `$channel` continue to read the passive `LexerActionExecutionContext` values and are not retroactively changed by writes in the same action body. Writes are produced only by accepted lexer action hooks; rejected paths and predicate-rejected paths do not leak writes. Other lexer attribute writes, compound or expression writes, unknown attributes, chained attributes, `ref`/`out` usage, and lexer predicate `$...` usage are outside the compatibility subset and are rejected deterministically by the transformer. `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` are deferred. Default/no-op transformer behavior, conservative `Parse(...)`, runtime-inline lexer behavior, and separate runtime lexer execution remain unchanged.

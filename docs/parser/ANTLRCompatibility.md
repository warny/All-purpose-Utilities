...
Only simple `$type = identifierOrString;` and `$channel = identifierOrString;` write statements are supported. They set bounded fields on `LexerActionExecutionResult`; `LexerEngine` applies those mutations to the accepted token before lexer commands, so `type(...)`, `channel(...)`, `skip`, and `more` keep their existing command/chunking authority. Other lexer attribute writes, compound or expression writes, unknown attributes, chained attributes, `ref`/`out` usage, and lexer predicate `$...` usage are outside the compatibility subset and are rejected deterministically by the transformer. `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` are deferred. Default/no-op transformer behavior, conservative `Parse(...)`, runtime-inline lexer behavior, and separate runtime lexer execution remain unchanged.

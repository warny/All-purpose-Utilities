...
The generated-C# opt-in write subset is deliberately bounded. Simple statement writes `$type = IDENTIFIER;`, `$type = "IDENTIFIER";`, `$channel = IDENTIFIER;`, and `$channel = "IDENTIFIER";` are rewritten to `SetLexerType(result, "...")` or `SetLexerChannel(result, "...")`. Generated hooks do not mutate `Token` directly; the helpers write only `LexerActionExecutionResult.TokenType` or `LexerActionExecutionResult.Channel`. `LexerEngine` builds the accepted token, runs accepted lexer actions, applies the requested `LexerActionExecutionResult` token type and channel to that token once, and then runs lexer commands. The ordering contract is therefore lexer action writes first and lexer commands second: `type(...)` overrides a previous `$type = ...`, `channel(...)` overrides a previous `$channel = ...`, `skip` remains authoritative over emission, and `more` keeps the existing chunking semantics. Reads such as `$type` and `$channel` remain reads of the passive `LexerActionExecutionContext`; writes in the same body do not mutate the context values read later by lexer attributes.

Regression coverage stabilizes last-write-wins behavior, `$type` plus `$channel` in the same action, multiple actions in one accepted token, contradictory writes, fragments, lexer rule references, quantifiers, `more` intermediate and final chunks, rejected alternatives, command override, whitespace and newline variations, identifier and string values including escaped strings, deterministic diagnostics for unsupported complex forms, and direct observation of `Token.RuleName` and `Token.Channel`.

Unsupported lexer attribute forms remain explicit: `$text = ...`, `$mode = ...`, `$line = ...`, `$pos = ...`, compound/coalescing/increment writes such as `$type += ...`, `$channel ??= ...`, `$type++`, and `$channel++`, dotted or chained writes, expression writes, embedded assignment expressions, `ref`/`out`, unknown attributes, and lexer predicate `$...` usage are rejected deterministically by the transformer. `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` are deferred. Default/no-op transformer behavior, conservative `Parse(...)`, runtime-inline lexer behavior, separate runtime lexer execution, general lexer action buffering/replay, general lexer rollback, and full ANTLR lexer compatibility remain unchanged and unsupported.

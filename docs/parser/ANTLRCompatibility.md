...
Lexer attribute writes, unknown attributes, chained attributes, `ref`/`out` usage, and lexer predicate `$...` usage are outside the compatibility subset and are rejected deterministically by the transformer. `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` are deferred. Default/no-op transformer behavior, conservative `Parse(...)`, runtime-inline lexer behavior, and separate runtime lexer execution remain unchanged.

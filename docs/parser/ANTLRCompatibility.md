...
- ANTLR-style `$...` convenience forms such as `$x.value`, `$xs.value`, `$rule.value`, `$param`, and `$local` are not core parser syntax; the optional C# embedded-code transformer rewrites only documented current-rule forms such as `$param`, `$local`, and declared bare `$returnName`, while child/labeled return conveniences remain unsupported and the default no-op transformer leaves all forms unchanged;
...
This is not child or labeled rule-call return access. `$child.value`, `$c.value`, `$x.value`, `$xs.value`, `$rule.value`, `$ctx`, `$label.ctx`, typed parser contexts, public ANTLR-style rule signatures, token attributes, lexer attributes, and general ANTLR attribute expressions remain unsupported. Explicit helper APIs such as `TryGetLabeledRuleCallReturn(...)` and `GetLabeledRuleCallReturns(...)` remain available for plain C# code, but the transformer does not create ANTLR-style child or labeled return syntax. The rewrite does not add implicit variables, typed label properties, automatic propagation, or public generated parser method changes. `Parse(...)` remains conservative and does not execute embedded code.

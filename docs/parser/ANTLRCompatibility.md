...
- execution-context rollback through the generated state manager is active for all parser backtracking attempt boundaries when lifecycle hooks are present; grammars with only inline actions or predicates continue to use the no-op state manager and do not get execution-context rollback;
...
**Utils.Parser (source-generator C# path)**: `@init` and `@after` are now supported as generated C# lifecycle hook methods executed through `ParseWithEmbeddedCode(...)` or an explicit-context `CreateRuntimePolicy(executionContext, basePolicy)`. `@init` fires at rule entry before any alternative is tried. `@after` fires after a successful rule result, before the result is committed. When lifecycle hooks are present in the grammar, the generated runtime policy installs a `GeneratedExecutionStateManager` that allows `ParserEngine` to capture and restore execution-context state around failed alternatives, quantifier iterations, and negation probes, so lifecycle mutations are rolled back on failure and committed only on success. Grammars without lifecycle hooks continue to use the no-op `NullParserExecutionStateManager`. Generated `Parse(...)` remains conservative and does not execute lifecycle hooks.

...
|---|---|---|
| Rule parameters `rule[int x]` | `Rule.Parameters` as raw text (each declaration split and name extracted lexically) | No argument passing or typed binding; generated C# lifecycle hooks can inspect parameter descriptors via `GetRuleParameterDescriptors` and read explicitly-supplied parameter values via `GetRuleParameter`/`TryGetRuleParameter`; frames are not auto-populated; `$param` and rule call arguments are not supported. |

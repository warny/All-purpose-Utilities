...
Generated C# opt-in contexts expose generic helpers `TryGetLabeledRuleCallResult`, `GetLabeledRuleCallResults`, `TryGetLabeledRuleCallReturn`, and `GetLabeledRuleCallReturns` for lifecycle and inline-action code. The list-return helper preserves call order, includes present-null values, and skips results where the requested return key is absent. These helpers perform ordinal lookup and no conversion or rule-name fallback. They do not add `$x`, `$x.value`, `$xs`, `$rule.value`, implicit variables, typed label/return properties, automatic return-to-parameter/local binding, expression rewriting, or lexer label/return support. Existing argument policies and conservative generated `Parse(...)` behavior are unchanged.

...
## Compatibility level estimate (non-normative)
...
The runtime supports common grammar forms, lexer modes, commands, project-compilation imports, token vocabularies, direct left recursion with limits, semantic predicates/actions through explicit opt-in paths, and bounded generated-C# lexer action/predicate support.
...
| Grammar-level actions `@header`, `@members`, `@footer`, etc. | `ParserDefinition.Actions` | Metadata only in the default runtime and runtime-inline path. In the source-generator C# path only, parser `@header` / `@parser::header` are injected near the top of the generated file, parser `@members` / `@parser::members` are injected into `{ClassName}ExecutionContext`, and parser `@footer` / `@parser::footer` are injected as trailing generated C# source near the end of the generated file. Lexer-scoped `@lexer::header`, `@lexer::members`, and `@lexer::footer` are injected through the same limited generated-C# compatibility model with dedicated lexer markers. Simple lexer actions/predicates and bounded lexer `$...` rewriting are separate generated-C# opt-in features; none of these surfaces implies complete ANTLR lexer target-language compatibility. See [`EmbeddedCodeExecutionModel.md`](./EmbeddedCodeExecutionModel.md). |
...
| Target-language action execution engines | No target-language engine is active by default. Inline parser actions and rule lifecycle hooks (`@init`/`@after`) can execute only through an explicit runtime policy or generated C# opt-in path. Parser and lexer header/member/footer compatibility blocks, plus simple lexer actions/predicates, exist only in the bounded generated-C# path and remain disabled in conservative `Parse(...)`. Unknown actions/scopes and broader target-language semantics remain non-executable. |
...
| `UP1029` EmbeddedCodeConstructNotExecutedByGenerator | Not emitted by runtime. | Emitted for visible embedded-code constructs outside the bounded generated hooks or supported parser/lexer header/member/footer injection points, including unsupported grammar actions, contextually invalid named actions, unknown action names, and unknown scopes. Supported simple lexer predicates/actions and supported `@lexer::header` / `@lexer::members` / `@lexer::footer` forms are not classified as unexecuted merely for being lexer-side. | Generator-only | Deterministic wording does not add execution or alter `ParserEngine`; all promoted hooks and injection points remain generated-C# opt-in, and conservative `Parse(...)` remains unchanged. |
...
When `UtilsParserEnableGeneratedRuleArgumentBinding=true`, the source generator validates certain generated-C# rule-call argument binding failures before emitting C#. When `UtilsParserEnableGeneratedRuleArgumentBinding=true`, static binding diagnostics cover uniquely resolved local and direct/transitive imported parser-rule targets that are present in the effective generated definition. Missing or ambiguous dependencies, imported-rule collisions, lexer-only targets, and unresolved calls do not produce `APU0107`. Aliased imports retain unqualified composition compatibility, but qualified `Alias.rule` syntax remains unsupported. `Parse(...)` remains conservative and `ParserEngine` remains authoritative.
...
Local entry rules mask imported rules. A diamond reaching the same declaration is deduplicated, while distinct imported declarations with the same unqualified name are recorded as a deterministic collision and the first declaration in source-order depth-first traversal remains effective for backward compatibility. Cycles expose a closed import path and fail with `UP0011`; missing dependencies fail with `UP0010`; ambiguous declared names fail with `UP0016`. Aliases are preserved on graph edges and continue to participate in unqualified runtime composition for compatibility, but qualified `Alias.rule` calls are unsupported.

Full-import and `tokenVocab` edges are distinct. Each planned edge exposes its declared kind and its effective kind after visibility propagation. Thus a declared full import below a lexer-only `tokenVocab` path is explicitly effective `TokenVocab`, across every descendant level; if another full-import path later upgrades the grammar, that traversal is explicitly represented as full. Provenance paths are retained separately for lexer-only and full visibility. Effective rules use the path that established their selected maximum visibility: parser rules always use a full path, lexer-only rules use the lexer-only path, and lexer rules reachable through both use the stable first full path alongside `IntroducedBy=FullImport`. `tokenVocab` contributes lexer rules/modes only. The entry grammar retains root, actions, and all effective options (`superClass`, `caseInsensitive`, `language`, and custom values); imported options/actions are provenance only and are not merged. `ParserDefinition.Imports` is descriptive metadata, not execution authority. Both the runtime project compiler and generator consume the plan. The generator mechanically projects its effective rules, modes, declared tokens/channels, provenance-selected order, and descriptive entry imports; it retains entry-owned root, options, and actions. `APU0107` uses the same effective target selection when its conclusion is certain.

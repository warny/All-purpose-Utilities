...
| Rule returns `returns [int x]` | `Rule.ReturnType` as raw text and `UP1007 RuleReturnsIgnored` | Recognized and preserved for passive descriptors; reported with `UP1007 RuleReturnsIgnored`; generated C# lifecycle hooks may explicitly read/write untyped return entries on the active frame through explicit helper methods, but returns are not typed, auto-allocated, exposed as implicit variables, or propagated to callers. |

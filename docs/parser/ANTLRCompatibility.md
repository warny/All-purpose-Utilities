...
Generated-C# opt-in lexer inline actions may use the optional C# ANTLR-style transformer for a deliberately small lexer attribute subset: `$text`, `$type`, `$channel`, and `$mode` reads. `EmbeddedLexerAttributeRewriter` rewrites these forms to generated helpers on the execution context, and `$text` specifically becomes `GetRequiredLexerText(context)` over passive `LexerActionExecutionContext.Text`. `$text` exposes the accepted token/chunk text available to the accepted lexer action context; tests lock this as context-level accepted text for fragment and lexer-rule-reference actions, not a guaranteed fragment-local or subrule-local slice. Simple tokens, quantified tokens, and alternatives read the accepted selected text. Actions paired with `skip`, `type(...)`, or `channel(...)` read text before the command suppresses, retags, or hides the token. With `more`, the action reads the current accepted chunk before final accumulation, and the later token action reads its own chunk text. `$type`, `$channel`, and `$mode` expose passive values available to that same action context before lexer commands mutate the emitted token. Lexer attribute writes, unknown attributes, chained attributes, `ref`/`out` usage, and lexer predicate `$...` usage are outside the compatibility subset and are rejected deterministically by the transformer. Default/no-op transformer behavior, conservative `Parse(...)`, runtime-inline lexer behavior, and separate runtime lexer execution remain unchanged.

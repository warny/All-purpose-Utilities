...
When `UtilsParserEnableGeneratedRuleArgumentBinding=true`, the source generator validates certain generated-C# rule-call argument binding failures before emitting C#. Static binding diagnostics validate only locally declared parser-rule targets. Imported parser-rule targets reached through direct or transitive same-project `.g4` `AdditionalFiles` imports remain metadata-only for `APU0107` because generated parser definitions currently contain local rules only and emit an empty `GrammarImport` collection. Missing imports, duplicate declared grammar names, ambiguous imported rule declarations, aliased imports, lexer-only rules, and unresolved calls likewise do not produce `APU0107`. This is a diagnostic-only extension: it does not merge grammar ASTs, make imports executable in a new way, alter runtime resolution, or change generated `Parse(...)`. The runtime remains authoritative for policies installed manually by callers and for all unsupported ANTLR4 delegate-grammar semantics.

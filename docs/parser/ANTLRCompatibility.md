...
Generated-C# opt-in lexer inline actions may use the optional C# ANTLR-style transformer for a deliberately small lexer attribute subset: `$text`, `$type`, `$channel`, and `$mode` reads. These forms are rewritten to generated helpers on the execution context and read passive runtime metadata from `LexerActionExecutionContext`. The metadata reflects the accepted lexer rule chunk before lexer commands and before `more` accumulation is joined to a later token. Lexer attribute writes, unknown attributes, chained attributes, `ref`/`out` usage, and lexer predicate `$...` usage are outside the compatibility subset and are rejected deterministically by the transformer. Default/no-op transformer behavior, conservative `Parse(...)`, and runtime-inline lexer behavior remain unchanged.

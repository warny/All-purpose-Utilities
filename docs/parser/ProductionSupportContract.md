...
| Direct/transitive imports, cycles, collisions, local masking, and local root | Project compiler uses the common deterministic composition plan; cycles/missing/ambiguous inputs diagnose; the entry grammar owns the root | The generator analyzes composition, but emitted definitions remain local-only | None | Runtime compiler: **Stable** within tested composition rules. Generator: **Preparatory/internal** and otherwise **out of scope** |
| `tokenVocab` | Lexer-only dependency visibility is composed when resolver inputs exist | Analyzed but imported effective definitions are not emitted | None | Runtime project compiler: **Stable** for controlled project inputs. Generator execution: **out of scope** |
...
| Concurrent execution | Separate runtime objects may be used according to their documented ownership; one parse operation is sequential | Static generated `Grammar` is cached; supplied execution contexts carry mutable state | Reusing a generated policy intentionally reuses its context | Parsing-internal parallelism is **out of scope**; callers must not share mutable execution context concurrently |
...
The generator project analyzes composition and shares the common plan, but `GrammarEmitter` does not yet emit the effective imported definition. Imported rules are therefore not executable through the generated class, and `APU0107` remains limited to locally declared targets that are actually resolved for generated binding.

Generated import composition is **Preparatory/internal** and **out of scope for production in `2.0.0-rc.1`** until the functional PR `feat(generator): emit effective imported grammar composition` is merged and this contract, compatibility documentation, diagnostics, and parity tests are updated together.

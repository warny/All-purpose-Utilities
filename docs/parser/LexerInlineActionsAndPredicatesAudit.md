...
The lexer owns operational state such as input position, current mode and mode stack, `more` accumulation, the token/chunk under construction, and match data used for acceptance. Parser memoization and `IParserExecutionStateManager` do not capture, restore, or hash that state. Reusing the parser manager or adding a generic `isLexer` switch would conflate two different execution models. A lexer-specific state contract should be considered only in a separate PR backed by a concrete need.

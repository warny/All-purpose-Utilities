...
## Lexer `$type` / `$channel` write edge-case stabilization

The generated-C# opt-in path supports only simple lexer action write statements for `$type` and `$channel`: identifier or string assignments such as `$type = IDENTIFIER;`, `$type = "IDENTIFIER";`, `$channel = HIDDEN;`, and `$channel = "HIDDEN";`. `EmbeddedLexerAttributeRewriter` rewrites those statements to `SetLexerType(result, "...")` or `SetLexerChannel(result, "...")`. Generated hooks receive `LexerActionExecutionContext context` and `LexerActionExecutionResult result`; they do not mutate `Token` directly. The helpers write only `LexerActionExecutionResult.TokenType` and `LexerActionExecutionResult.Channel`.

`LexerEngine` constructs the accepted token, executes accepted lexer actions, applies `result.TokenType` and `result.Channel` to that token, and then executes lexer commands. This locks the write-before-command contract: action writes happen first, commands remain authoritative, `type(...)` can override a previous `$type = ...`, `channel(...)` can override a previous `$channel = ...`, `skip` remains authoritative over emission, and `more` keeps the existing chunking semantics for intermediate and final chunks.

Reads remain context-passive even when a write appears earlier in the same body. `$type` and `$channel` read `LexerActionExecutionContext.TokenType` and `LexerActionExecutionContext.Channel`; they do not read back pending `LexerActionExecutionResult` mutations. Regression coverage stabilizes last-write-wins, `$type` plus `$channel` in the same action, multiple actions in one accepted token, contradictory writes, fragments, lexer rule references, quantifiers, rejected alternatives, whitespace and newline variations, identifier/string/escaped-string values, command override, direct `Token.RuleName` and `Token.Channel` observation, and deterministic diagnostics for unsupported complex forms.

Unsupported forms remain unsupported and diagnostic-only: `$text = ...`, `$mode = ...`, `$line = ...`, `$pos = ...`, `$type += ...`, `$channel ??= ...`, `$type++`, `$channel++`, dotted/chained writes, expression writes, embedded assignment expressions, `ref`/`out`, lexer `$...` in predicates, runtime-inline lexer execution, separate runtime lexer execution, general lexer action buffering/replay, general lexer rollback, and full ANTLR lexer compatibility.

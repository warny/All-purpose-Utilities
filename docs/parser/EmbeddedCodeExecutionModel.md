...
## Limited read-only return attribute rewrite

Before generated parser action and lifecycle bodies are emitted, a lexical scanner rewrites the supported parser attribute reads outside C# strings, character literals, and comments. Canonical `$identifier.identifier` return reads resolve the root with ordinal matching: the exact current parser-rule name first, then a rule-wide visible assignment or list rule-reference label. A name present in both label namespaces is rejected as ambiguous. Target return names are checked against preserved return descriptors. Assignment/current-rule expressions call required helpers and return `object?`; list-label expressions call `GetLabeledRuleCallReturns(...)` and return `IReadOnlyList<object?>`. List projection preserves successful order, includes present-null entries, skips missing returns, and is empty when the label has no successful calls. Bare `$name` is separate from label reads and resolves only to current-rule parameters/locals; it inspects descriptor `RawType` solely to emit a typed helper call, without conversion, implicit variables, or typed generated fields/properties.

Inline actions and `@after` support both forms. Current-rule reads are also allowed in `@init`, but assignment- and list-label reads are diagnosed because the child has not executed. Predicate attribute references are diagnosed in this increment. Assignment labels declared in another alternative are accepted rule-wide, so runtime access can throw `ParserAttributeAccessException` when the selected path did not produce that label. Present-null values remain distinguishable from absence. Both label forms are read-only; writes are diagnosed, bare labels and special indexing syntax are unsupported, writes remain explicit through `SetRuleReturn(...)`, lexer code is excluded, and conservative `Parse(...)` is unchanged.

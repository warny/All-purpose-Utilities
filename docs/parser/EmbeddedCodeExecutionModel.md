...
Before generated parser action and lifecycle bodies are emitted, a lexical scanner rewrites only canonical `$identifier.identifier` return reads outside C# strings, character literals, and comments. The root is resolved with ordinal matching: the exact current parser-rule name first, then a rule-wide visible assignment or list rule-reference label. A name present in both label namespaces is rejected as ambiguous. Target return names are checked against preserved return descriptors. Assignment/current-rule expressions call generic required helpers and return `object?`; list-label expressions call `GetLabeledRuleCallReturns(...)` and return `IReadOnlyList<object?>`. List projection preserves successful order, includes present-null entries, skips missing returns, and is empty when the label has no successful calls. No `RawType` inspection, conversion, implicit variable, or label-specific API is generated.

...
Before generated parser action and lifecycle bodies are emitted, a lexical scanner rewrites only canonical `$identifier.identifier` return reads outside C# strings, character literals, and comments. The root is resolved with ordinal matching: the exact current parser-rule name first, then a rule-wide visible assignment rule-reference label. Target return names are checked against preserved return descriptors. The generated expressions call generic `GetRequiredRuleReturn(...)` or `GetRequiredLabeledRuleCallReturn(...)` helpers and return `object?`; no `RawType` inspection, conversion, implicit variable, or label-specific API is generated.

Inline actions and `@after` support both forms. Current-rule reads are also allowed in `@init`, but assignment-label reads are diagnosed because the child has not executed. Predicate attribute references are diagnosed in this increment. Assignment labels declared in another alternative are accepted rule-wide, so runtime access can throw `ParserAttributeAccessException` when the selected path did not produce that label. Present-null values remain distinguishable from absence. List labels use `GetLabeledRuleCallReturns(...)`, writes remain explicit through `SetRuleReturn(...)`, lexer code is excluded, and conservative `Parse(...)` is unchanged.

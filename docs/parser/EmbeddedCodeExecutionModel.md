...
Parameters, child return access such as `$child.value`, labeled rule-call returns such as `$c.value`, list-labeled projections, lexer attributes, `ref`/`out`, semantic predicates, `@init`, and dotted current-rule return writes such as `$rule.value = ...` remain unsupported/read-only. Use bare `$returnName = ...` only for declared current-rule return attributes when opting into the C# transformer.

...
This support includes only the optional generated-C# lexer action read rewrite documented below for `$text`, `$type`, `$channel`, and `$mode`; it does not add lexer predicate attributes, lexer attribute writes, runtime-inline lexer execution, a separate runtime lexer, generalized action buffering/replay, complete ANTLR mode/channel/command semantics, or rollback of external side effects performed by lexer actions.
...
Lexer attribute writes other than simple `$type = ...;` and `$channel = ...;` statements are intentionally unsupported and produce deterministic transformer diagnostics. Supported writes are applied after accepted action execution and before lexer commands, so commands remain authoritative. Lexer `$...` attributes in lexer predicates are unsupported in this phase and also produce deterministic diagnostics. The no-op transformer preserves lexer `$...` source unchanged, conservative `Parse(...)` does not execute lexer hooks, runtime-inline lexer execution remains unsupported, and no C# target-language rewriting exists in `LexerEngine` or `ParserRuntimeFeaturePolicy`.

Future lexer attribute candidates are limited to values the runtime can expose without changing authority boundaries. `$line` and `$pos` are possible future read-only candidates. `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` are explicitly deferred and must not be inferred from this read-only metadata support.

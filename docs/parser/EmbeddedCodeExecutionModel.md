...
- Automatic model-wide preparation and registry population are not implemented; callers may invoke the explicit registry builder and must configure the returned registry explicitly. Skips remain limited to constructs outside this runtime-inline path, such as grammar-level actions, rule lifecycle actions, and non-inline actions.

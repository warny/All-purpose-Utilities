...
The optional C# ANTLR-style transformer owns the limited lexer `$...` rewrite for generated-C# opt-in lexer inline actions through `EmbeddedLexerAttributeRewriter`. The supported lexer action reads are `$text`, `$type`, `$channel`, and `$mode`; they are rewritten before hook emission to generated execution-context helpers that read passive `LexerActionExecutionContext` metadata.

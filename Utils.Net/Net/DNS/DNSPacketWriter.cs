...
/// </summary>
/// <remarks>
/// The <see cref="DNSPacketWriter"/> scans assemblies (via <see cref="DNSFactory"/>) for <see cref="DNSResponseDetail"/>
...
public class DNSPacketWriter : IDNSWriter<byte[]>
{
    ...
    /// <summary>
    /// Gets the default <see cref="DNSPacketWriter"/> instance, which scans the DNS types
    /// from <see cref="DNSFactory.Default"/>.
    /// </summary>
    public static DNSPacketWriter Default { get; } = new DNSPacketWriter(DNSFactory.Default);

    /// <summary>
    /// Initializes a new instance of the <see cref="DNSPacketWriter"/> class, combining
    ...
    public DNSPacketWriter(params DNSFactory[] factories)
        : this((IEnumerable<DNSFactory>)factories)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="DNSPacketWriter"/> class, scanning the given
    ...
    private sealed class Datas
    {
        ...
        {
            // If the name has already been written, write a pointer reference (0xC000 + position).
            ...
            // Otherwise, record this name's position and write it out label by label.

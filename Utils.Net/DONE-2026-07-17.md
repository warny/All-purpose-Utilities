...
- ✅ Fuzz oversized lines, command floods, endless SMTP DATA, endless NNTP POST and multiline client replies. (`SmtpServerSecurityTests`, `NntpServerLimitTests`, `CommandResponseServerTests.MaxLineLength_*`)
- ✅ Verify TLS-gated SMTP authentication and fail-closed local-domain configuration. (`SmtpServerSecurityTests`)
- ✅ Test authentication throttling and session-state isolation across reused instances. (`SmtpServerSecurityTests`, `Pop3ServerSecurityTests`)
- Reject spoofed/malformed NTP and DNS replies; test timeout/cancellation and DNS TCP fallback.
- ✅ Fuzz DNS with truncated headers, inflated counts, invalid RDLENGTH, reserved labels, pointer loops, uncached pointers and cross-RDATA reads. (`DNSPacketReaderBoundaryTests`)
...
- ✅ Verify handler exceptions and concurrent output behavior. (`CommandResponseServerTests.CloseAfterResponse_*`, `MaxLineLength_*`)

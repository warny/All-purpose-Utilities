...
Lexer `$...` conveniences must follow the same optional-transformer model as parser attributes. Keep lexer attribute rewriting in the C# embedded-code transformer and dedicated rewriter code, not in `LexerEngine`, `ParserEngine`, or `ParserRuntimeFeaturePolicy`. Supported generated-C# lexer action reads are intentionally narrow (`$text`, `$type`, `$channel`, `$mode`); `$text` must continue to mean `LexerActionExecutionContext.Text` read through generated helpers, not a guaranteed fragment-local slice. Writes and lexer predicate attributes must remain deterministic diagnostics unless a documented runtime-neutral action-result model is added.

...
The generated-C# opt-in embedded-code path has a bounded lexer `$...` rewrite architecture through the optional C# transformer and `EmbeddedLexerAttributeRewriter`. Inline lexer actions support read-only `$text`, `$type`, `$channel`, `$mode`, `$line`, and `$pos` helpers. `$text` reads `LexerActionExecutionContext.Text` via `GetRequiredLexerText(context)` and is locked by tests as accepted token/chunk context text: fragments and lexer rule references receive context-level accepted text, `skip`/`type(...)`/`channel(...)` read before commands apply, and `more` reads the current accepted chunk before final accumulation. `$type`, `$channel`, and `$mode` read `LexerActionExecutionContext.TokenType`, `Channel`, and `Mode` through `GetRequiredLexerType(context)`, `GetRequiredLexerChannel(context)`, and `GetRequiredLexerMode(context)`. `$line` and `$pos` read `LexerActionExecutionContext.Line` and `Column` through `GetRequiredLexerLine(context)` and `GetRequiredLexerPos(context)`, using 1-based `SourceSpan.Line` and `SourceSpan.Column` for the beginning of the accepted token/chunk; `$pos` is not documented as full ANTLR `charPositionInLine` compatibility. These passive values are available after accepting the token/chunk and before lexer commands apply, so `type(...)`, `channel(...)`, `mode(...)`, `pushMode(...)`, and `popMode` do not alter what the current action reads. Fragment and lexer-rule-reference actions read the available accepted outer token metadata. `$type`/`$channel` simple writes are implemented for generated-C# opt-in lexer inline actions only: simple `$type = identifierOrString;` and `$channel = identifierOrString;` statements rewrite to helpers that set bounded `LexerActionExecutionResult` fields. Mutations apply after accepted action execution and before lexer commands, so commands remain authoritative. Still future/out of scope: `$text` writes, `$mode` writes, `$line`/`$pos` writes, a general mutable lexer action result, predicate attributes, runtime-inline lexer execution, and full ANTLR lexer compatibility. `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` remain deferred.

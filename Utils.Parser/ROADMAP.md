...
- Limited ordinary parser alternative rollback is active. Complete ANTLR transactional semantics and action buffering are not active. `@init` / `@after` remain unsupported. Lexer actions and predicates remain unsupported. Completed-result memoization is semantic-state-aware, and restored alternatives restore the state key before later cache lookups.

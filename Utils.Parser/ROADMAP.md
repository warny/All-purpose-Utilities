...
- rule `locals [...]` clauses now emit deterministic explicit compatibility diagnostics (`UP1008 RuleLocalsIgnored`) instead of generic silent metadata discard; their raw declarations are preserved as passive runtime metadata, and generated C# execution contexts expose explicit lifecycle helpers for `context.InvocationFrame` local-store access without automatic allocation or typed local generation.

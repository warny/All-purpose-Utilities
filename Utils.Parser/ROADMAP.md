...
- Explicit parser rule-call execution policy infrastructure is available through `IParserRuleCallExecutionPolicy`, `NullParserRuleCallExecutionPolicy`, and `ParserRuleCallExecutionContext`. `ParserEngine` invokes `BeforeRuleCall(...)`, parses the child, applies current-call-site raw argument and label annotations, and then invokes `AfterRuleCall(...)` with success/failure and the completed tracked call result when available. `ParserRuntimeFeaturePolicy.Default` installs the no-op policy. Generated C# preserves a custom rule-call policy from `basePolicy` through `CreateRuntimePolicy(executionContext, basePolicy)` and the new backward-compatible `ParseWithEmbeddedCode(input, executionContext, basePolicy)` overload. This remains an explicit extension point only: no `callee[...]` evaluation, automatic positional/named binding, automatic seed, `$param`, `$x`, `$x.value`, `$rule.value`, typed generated variables, or label-backed parse-node storage is implemented. Policy external side effects are not automatically rolled back; current-call-site result metadata remains rollback/memoization safe.

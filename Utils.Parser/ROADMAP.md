...
- The generated-C# opt-in lexer attribute surface is consolidated for the current bounded subset. Inline lexer actions may read `$text`, `$type`, `$channel`, `$mode`, `$line`, and `$pos`; `$line`/`$pos` expose the accepted token/chunk start `SourceSpan.Line`/`SourceSpan.Column`, with `$pos` documented as this runtime's 1-based column rather than full ANTLR `charPositionInLine` compatibility. Inline lexer actions may write only simple `$type = ...` and `$channel = ...` statements with identifier or string values. Those writes flow through `LexerActionExecutionResult`, are applied to the accepted token before lexer commands, and are covered by edge-case tests for last-write-wins, same-action type/channel writes, fragments, lexer rule references, quantifiers, rejected alternatives, `more`, command override, passive read-context behavior, and deterministic diagnostics for complex unsupported forms.
- Future/non-goal lexer semantics remain explicit: `$mode = ...`, `$text = ...`, `$line = ...`, `$pos = ...`, `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, `$input`, lexer predicate `$...`, runtime-inline lexer execution, a separate runtime lexer, full ANTLR lexer compatibility, general lexer action replay/buffering, and general lexer rollback are unsupported.

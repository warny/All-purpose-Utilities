...
- `ParserExecutionContextCopier<TContext>` is available as a preparatory runtime copy primitive for generated execution-context snapshot/fork/commit work, and generated execution contexts now expose internal `Fork()` and `CopyFrom(...)` helpers that delegate to it. `IParserExecutionStateManager` is available through `ParserRuntimeFeaturePolicy`; the default manager is no-op, and generated policies install a manual manager backed by `Fork()` / `CopyFrom(...)`. It is not yet wired into `ParserEngine` rollback, action buffering, `@init`, or `@after`.

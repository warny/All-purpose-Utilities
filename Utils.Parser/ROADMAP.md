...
- `QuantifierNode` is a public subtype of `ParserNode` produced by `TryParseQuantifier` when a sequence element is a quantifier (`?`, `*`, `+`). Its `Rule` carries the owning parser rule (same as the enclosing sequence), and its `Children` list contains one entry per matched repetition of the quantifier body. It is never produced by named-rule matching. `QuantifierNode` is a `ParserNode` subtype so that integer-index navigation (`nav[i]`) and `RawChildren` treat it as a transparent direct child.
- `ParseTreeNavigator.TryChild(string)` and `Children(string)` navigate through `QuantifierNode` wrappers transparently using a two-pass strategy: direct children matching the requested name are returned first (highest priority); only when no direct match exists are `QuantifierNode` contents searched. Inside a `QuantifierNode`, the search recurses through synthetic same-rule `ParserNode` frames produced by `TryParseSequence` for grouped quantifiers such as `(a b)?`, so every named rule is reachable by name regardless of nesting depth. Nested `QuantifierNode` wrappers are also traversed recursively. Index-based navigation and `Descendants()` / `Descendants(string)` are unaffected.
...
- the remaining embedded-code work is explicit: any richer lexer predicate/action semantics, any separately justified buffering or rollback mechanism, a broader ANTLR corpus, and ongoing protection of the still-separate generator `G4Grammar` collector and `EmbeddedCodeRuntimeDiscovery` implementations. Structural parity is now locked by executable tests covering ordinary parser traversal, direct left recursion, lexer rules and modes, deterministic discovery order, and dispatch alternative/element indexes; the collectors remain tailored to their different models rather than fused.
...
#### Lexer embedded-code transactional state design

**Status: complete.**

The lexer state and transaction boundary is now documented as a design contract. Predicates explore recognition paths without a supported managed mutation surface; actions execute only after token acceptance with a fresh acceptance-local `LexerActionExecutionResult`; bounded `TokenType`, `Channel`, and `Mode` requests are applied before authoritative lexer commands; and lexer-owned operational state lifetimes remain distinct from parser-managed snapshots and memoization. The runtime lexer characterization suite now locks the missing state-boundary invariants for false predicates, losing rules, rejected alternatives, failed sequences, source-ordered accepted actions, fresh per-acceptance action results, last-write-wins action mutations, command precedence, `$mode` stack-depth isolation, `skip`, and `more`. This completed audit does not implement a general lexer snapshot manager, make user effects rollback-safe, add runtime-inline lexer execution, or change lexer behavior. Future work should remain tied to concrete behavior gaps or generated-C# parity needs rather than introducing speculative transaction infrastructure.
...
The generated-C# rule-call argument binding option now benefits from bounded static diagnostics for locally declared parser-rule targets and uniquely resolved parser-rule targets reached through direct or transitive `.g4` imports supplied as same-project `AdditionalFiles`. The project index is declared-grammar-name based, local rules shadow imported rules, missing imports, ambiguous imported declarations, and aliased imports stay metadata-only, and lexer-only rules are not binding targets. This does not complete Phase 7 and does not expand supported syntax. Future work remains for complete ANTLR delegate-grammar semantics, alias-qualified calls if ever represented, named arguments, expressions, user-defined types, and C# semantic analysis.

...
- Successful child parser-rule calls now capture immutable return snapshots after child `@after` and can be retained in the parent invocation frame by rule-reference label. Assignment labels use last-successful-result-wins; list labels append successful results in order. The generated C# opt-in path exposes generic assignment/list result and return helpers, including present-null versus absent semantics and ordered missing-key-skipping list projection. Labeled state participates in managed rollback, memoization restoration, and conservative volatile hashing for unsupported return objects; current call-site labels are reapplied after memoized calls. This does not add ANTLR `$x`, `$x.value`, `$xs`, `$rule.value`, implicit variables, automatic return binding, or lexer returns, and conservative `Parse(...)` is unchanged.

...
- Simple lexer inline actions such as `A : 'a' { OnLex(); } ;` are supported only by generated-C# opt-in policies. They execute after the owning token is accepted; conservative `Parse(...)` does not execute them. The tested boundary includes actions reached through lexer rule references and fragments, actions in simple quantifiers, duplicate action source text at distinct positions, and interaction with already-supported lexer commands such as `skip`.

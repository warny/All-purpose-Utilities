...
The generated-C# opt-in embedded-code path has a bounded lexer `$...` rewrite architecture through the optional C# transformer and `EmbeddedLexerAttributeRewriter`. Inline lexer actions support read-only `$text`, `$type`, `$channel`, and `$mode` helpers. `$text` reads `LexerActionExecutionContext.Text` via `GetRequiredLexerText(context)` and is locked by tests as accepted token/chunk context text: fragments and lexer rule references receive context-level accepted text, `skip`/`type(...)`/`channel(...)` read before commands apply, and `more` reads the current accepted chunk before final accumulation. `$type`, `$channel`, and `$mode` read `LexerActionExecutionContext.TokenType`, `Channel`, and `Mode` through `GetRequiredLexerType(context)`, `GetRequiredLexerChannel(context)`, and `GetRequiredLexerMode(context)`. These passive values are available after accepting the token/chunk and before lexer commands apply, so `type(...)`, `channel(...)`, `mode(...)`, `pushMode(...)`, and `popMode` do not alter what the current action reads. Fragment and lexer-rule-reference actions read the available accepted outer token metadata. Lexer attribute writes and lexer predicate attributes remain unsupported with deterministic diagnostics. Future candidates may include read-only `$line` and `$pos`; `$index`, `$int`, `$token`, `$start`, `$stop`, `$ctx`, and `$input` remain deferred. Future work must keep runtime-inline lexer execution and full ANTLR lexer attribute compatibility out of scope until a dedicated mutable action-result design exists.

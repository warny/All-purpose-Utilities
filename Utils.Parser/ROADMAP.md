...
Generated-C# labeled child return access is currently stabilized through explicit helpers over existing invocation-frame `ParserRuleCallResult` storage. Current-rule returns may use bare `$value` only in the declaring rule through the optional C# transformer, while parent access to `c=child` and `xs+=child` returns remains `GetRequiredLabeledRuleCallReturn`, `TryGetLabeledRuleCallReturn`, `TryGetLabeledRuleCallResult`, `GetLabeledRuleCallResults`, and `GetLabeledRuleCallReturns`. Present-null values, missing labels, missing return names, list-label ordering, rollback, and memoized call-site label rebinding are covered before any syntax sugar. Assignment-label `$c.value`/`$x.value` sugar is supported only in generated-C# inline parser actions and `@after`; `$xs.value`, `$child.value`, `$rule.value`, `$ctx`, typed parser contexts, and public ANTLR-style parser rule methods remain out of scope; any future sugar must stay inside `IParserEmbeddedCodeTransformer` and must not move C# logic into `ParserEngine`.

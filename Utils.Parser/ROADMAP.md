...
- the remaining embedded-code work is explicit: any richer lexer predicate/action semantics, any separately justified buffering or rollback mechanism, deeper alignment between the generator `G4Grammar` collector and `EmbeddedCodeRuntimeDiscovery`, and a broader ANTLR corpus. The current lexer acceptance and state boundary is documented below and does not require a general snapshot design.

...
- `ParserExecutionContextCopier<TContext>` is available as a public runtime helper for parser execution-context snapshot/fork/commit. Generated execution contexts expose it through internal `Fork()` and `CopyFrom(...)` helpers. Generated `ParserRuntimeFeaturePolicy` instances expose a manual `IParserExecutionStateManager` backed by those helpers; the generated state manager is installed in the policy only when the grammar declares at least one lifecycle hook. `ParserEngine` calls `Capture()` and `Restore()` on the state manager during branch attempts (alternatives, quantifiers, negation probes) when lifecycle hooks are active. Grammars without lifecycle hooks use the no-op manager and parser behavior is unchanged. `IParserRuleLifecycleExecutor` executes `@init` and `@after` lifecycle hooks in the source-generator C# opt-in path.
...
- Steps 1–6 are complete for the source-generator C# opt-in path. Managed execution-state rollback is active for all parser backtracking attempt boundaries. Lifecycle hooks fire through the configured `RuleLifecycleExecutor`; grammars without hooks use the no-op executor. Action buffering and external side-effect rollback are not implemented. Completed-result memoization is semantic-state-aware, memoization hits restore stored post-rule state snapshots, and restored parser attempts restore the state key before later cache lookups.
- Lexer actions, lexer predicates, unsupported grammar actions, `@lexer::members`, and automatic default execution remain not done and must not be documented as complete.

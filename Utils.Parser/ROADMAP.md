...
- The first concrete policy, `PositionalLiteralRuleCallExecutionPolicy`, is available as an explicit opt-in while the default remains metadata-only.
- It binds exact-arity positional simple literals to declared parser-rule parameter names after whole-call validation; declared C# types, named binding, and arbitrary expressions remain unsupported.
- Binding uses rollback-managed pending child seeds. Generated state-aware memoization deterministically includes the supported literal values (`null`, `bool`, `int`, `long`, `double`, `string`, and `char`) so different bound values cannot reuse stale child state.
- Future work may consider typed validation, named binding, return/label semantics, and broader expression semantics only through separately reviewed phases. Lexer execution and ANTLR-complete argument semantics remain out of scope.

...
- The runtime now also includes structural shared-prefix execution planning metadata that associates shared-token observations with continuation descriptors. These plans are not executed and do not alter parser behavior.
- During deterministic alternative orchestration, the scheduler now computes those structural shared-prefix plans as informational metadata only; there is still no shared-prefix execution, continuation scheduling/resumption, adaptive prediction, or parallel parsing.

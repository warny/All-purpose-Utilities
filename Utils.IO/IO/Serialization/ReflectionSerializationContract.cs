...
internal sealed record SerializableMemberContract(MemberInfo Member, Type ValueType, int Order, Type? CodecType = null, Type? FramingType = null);
...
internal static class ReflectionContractBuilder
{
    ...
    private static SerializableMemberContract CreateValidatedMember(MemberInfo member, Type valueType, int order, SerializationDirection direction, List<SerializationContractDiagnostic> diagnostics)
    {
        ...
        return new(member, valueType, order, codecType, framingType);
    }
}

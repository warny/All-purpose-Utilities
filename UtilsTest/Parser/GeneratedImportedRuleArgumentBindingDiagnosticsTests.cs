...
public sealed class GeneratedImportedRuleArgumentBindingDiagnosticsTests
{
    ...
    /// <summary>Verifies project-wide option changes reuse parsed grammar files while rerunning project validation.</summary>
    [TestMethod]
    public void IncrementalTracking_GlobalBindingOptionChange_ReusesParsedGrammarFiles()
    {
        var compilation = CreateCompilation();
    ...
    /// <summary>Verifies editing an imported grammar reparses only that file while project validation observes the new signature.</summary>
    [TestMethod]
    public void IncrementalTracking_ImportedFileChange_ReparsesOnlyImportedFile()
    {
        var compilation = CreateCompilation();
        var root = Grammar("Root.g4", "parser grammar Root; import Shared; start : child[1] ;");
        var shared = Grammar("Shared.g4", "parser grammar Shared; child[int value] : TOKEN ;");
    ...
    /// <summary>Verifies editing the caller grammar reparses only the caller and refreshes binding diagnostics.</summary>
    [TestMethod]
    public void IncrementalTracking_CallerFileChange_ReparsesOnlyCallerFile()
    {
        var compilation = CreateCompilation();
        ...
        driver = driver.ReplaceAdditionalText(root, Grammar("Root.g4", "parser grammar Root; import Shared; start : child[1] ;")).RunGenerators(compilation);
    ...
    [TestMethod]
    public void IncrementalTracking_IndependentFileChange_ReparsesOnlyIndependentFile()
    {
        var compilation = CreateCompilation();
        var root = Grammar("Root.g4", "parser grammar Root; import Shared; start : child[bad] ;");
        var shared = Grammar("Shared.g4", "parser grammar Shared; child[int value] : TOKEN ;");
    ...
    [TestMethod]
    public void IncrementalTracking_MetadataChange_ReparsesOnlyChangedFile()
    {
        var compilation = CreateCompilation();
        var root = Grammar("Root.g4", "parser grammar Root; import Shared; start : child[1] ;");
        var shared = Grammar("Shared.g4", "parser grammar Shared; child[int value] : TOKEN ;");
    ...
    /// <summary>Verifies adding and removing an imported file with one driver updates parsing, resolution, diagnostics, and generated sources.</summary>
    [TestMethod]
    public void IncrementalTracking_AddAndRemoveImport_RefreshesResolutionWithoutStaleState()
    {
        var compilation = CreateCompilation();
        var root = Grammar("Root.g4", "parser grammar Root; import Shared; start : child[bad] ;");
        var shared = Grammar("Shared.g4", "parser grammar Shared; child[int value] : TOKEN ;");
    ...
    /// <summary>Reads a non-public generated pipeline property by name for test assertions.</summary>

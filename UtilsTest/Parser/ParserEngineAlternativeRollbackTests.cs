...
public class ParserEngineAlternativeRollbackTests
{
    ...
    [TestMethod]
    public void MemoizedRuleHit_RestoresPostRuleExecutionStateAfterAlternativeRollback()
    {
        var manager = new CounterExecutionStateManager();
        var childRule = ParserRule("child", 1, [Alternative(0, Sequence([RuleRef("A"), Action("bump")]))]);
        var startRule = CreateStartRule([
            Alternative(0, Sequence([RuleRef("child"), RuleRef("B")])),
            Alternative(1, RuleRef("child"))
        ]);
        var parser = CreateParser(startRule, manager, parserRules: [startRule, childRule]);
        var diagnostics = new DiagnosticBag();
    ...
    [TestMethod]
    public void MemoizedRuleHit_WithNullExecutionStateManager_DoesNotReplayActions()
    {
        var counter = 0;
        var executor = new DelegatingActionExecutor(_ =>
        {
    ...
    [TestMethod]
    public void SuccessfulAlternativeCandidates_CommitOnlyWinnerExecutionStateSnapshot()
    {
        var manager = new CounterExecutionStateManager();
        var parser = CreateParser(
            CreateStartRule([
                Alternative(0, Sequence([RuleRef("A"), Action("bump")])),
                Alternative(1, Sequence([RuleRef("A"), Action("bump"), Action("bump")]))
            ]),
            manager);

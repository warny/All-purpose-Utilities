...
public class Antlr4GrammarGeneratorTests
{
    ...
    [TestMethod]
    public void GrammarEmitter_Emit_ForRepresentativeGrammar_ProducesStableSourceShape()
    {
        var src = Emit("""
            grammar Sample;
            @parser::members { private int _count; }
            start : item+ EOF ;
            item : ID | NUMBER | 'x' { _count++; } ;
            ID : [a-zA-Z]+ ;
            NUMBER : [0-9]+ ;
            WS : [ \t\r\n]+ -> skip ;
            """, "Generated.Tests", "SampleGrammar", "Sample.g4");
        ...
        AssertContainsInOrder(
            ...
            "internal static partial class SampleGrammar",
            "public static ParserDefinition BuildDefinition()",
            "var _ID = new Rule",
            "var _NUMBER = new Rule",
            "var _start = new Rule",
            "new Quantifier(new RuleRef(\"item\"), 1, null)",
            "new LiteralMatch(\"x\")",
            "public static ParseNode Parse",
            "public static ParserDefinition Build()",
            "internal sealed partial class SampleGrammarExecutionContext",
            "private int _count;");
    }
    ...
    private static void AssertContainsInOrder(string source, params string[] fragments)
    {
        ...
        {
            int nextIndex = source.IndexOf(fragment, currentIndex, StringComparison.Ordinal);
            Assert.IsTrue(nextIndex >= 0, $"Expected to find fragment after index {currentIndex}: {fragment}");

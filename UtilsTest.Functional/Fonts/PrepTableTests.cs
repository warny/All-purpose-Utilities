...
public class PrepTableTests
{
    ...
    private static PrepTable NewTable() => (PrepTable)new TrueTypeFont(0).CreateTable(TableTypes.PREP);

    [TestMethod]
    public void RoundTrip_PreservesInstructionBytes()
    {
        var source = NewTable();
        source.Instructions = [0x01, 0x02, 0xFF, 0x00];

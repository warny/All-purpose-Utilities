...
internal static class GrammarEmitter
{
    ...
    /// <summary>
    /// Emits generated runtime policy helpers plus predicate/action dispatchers for embedded parser code.
    /// </summary>
    ...
    private static void EmitEmbeddedCodeRuntime(StringBuilder sb, IReadOnlyList<EmbeddedCodeHook> hooks, string className)
    {
        ...
        sb.AppendLine("    /// <summary>Creates a runtime feature policy that executes generated C# hooks for parser semantic predicates and inline parser actions.</summary>");
        sb.AppendLine("    /// <param name=\"basePolicy\">Optional policy whose non-embedded-code components are preserved.</param>");
        sb.AppendLine("    /// <returns>A runtime policy using this generated grammar's embedded-code hooks.</returns>");
        sb.AppendLine("    public static ParserRuntimeFeaturePolicy CreateRuntimePolicy(ParserRuntimeFeaturePolicy? basePolicy = null)");
        sb.AppendLine("    {");
        sb.AppendLine("        return (basePolicy ?? ParserRuntimeFeaturePolicy.Default) with");
        sb.AppendLine("        {");
        sb.AppendLine("            SemanticPredicateEvaluator = new GeneratedSemanticPredicateEvaluator(),");
        sb.AppendLine("            ParserActionExecutor = new GeneratedParserActionExecutor()");
        sb.AppendLine("        };");
        sb.AppendLine("    }");
        sb.AppendLine();
        ...
        sb.AppendLine("    }");
        sb.AppendLine();
        EmitSemanticPredicateEvaluator(sb, predicates);
        EmitParserActionExecutor(sb, actions);
    ...
    private static void EmitSemanticPredicateEvaluator(StringBuilder sb, IReadOnlyList<EmbeddedCodeHook> predicates)
    {
        ...
        foreach (var predicate in predicates)
        {
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                return {predicate.MethodName}(context) ? SemanticPredicateEvaluationOutcome.Satisfied : SemanticPredicateEvaluationOutcome.Rejected;");
            sb.AppendLine("            }");
            sb.AppendLine();
        }

        sb.AppendLine("            return SemanticPredicateEvaluationOutcome.NotEvaluated();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
    ...
    private static void EmitParserActionExecutor(StringBuilder sb, IReadOnlyList<EmbeddedCodeHook> actions)
    {
        ...
        foreach (var action in actions)
        {
            ...
            sb.AppendLine("            {");
            sb.AppendLine($"                {action.MethodName}(context);");
            sb.AppendLine("                return ParserActionExecutionOutcome.Executed;");
            sb.AppendLine("            }");
            sb.AppendLine();
        }

        sb.AppendLine("            return ParserActionExecutionOutcome.NotExecuted();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
    }

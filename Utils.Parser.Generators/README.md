...
When `UtilsParserEnableGeneratedRuleArgumentBinding=true`, the generator performs a bounded static validation pass for generated-C# positional rule-call argument binding. Static binding diagnostics validate only parser-rule targets declared in the same `.g4` file as the call site. The generator still parses and preserves `import` directives and maintains an internal same-project import resolver for preparatory analysis, but generated classes currently emit `ParserDefinition` instances with local rules only and `Imports: new GrammarImport[0]`. Therefore imported, transitive, missing, duplicate-name, ambiguous, aliased, lexer-only, and unresolved targets remain metadata-only for generated-binding diagnostics and do not produce `APU0107`. This keeps the diagnostic surface aligned with the generated definition that will be executed. This pass does not merge grammar ASTs, does not make imports executable in a new way, does not change `Parse(...)`, and does not claim complete ANTLR4 delegate-grammar compatibility.

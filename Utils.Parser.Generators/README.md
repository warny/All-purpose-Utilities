...
Generated-C# opt-in lexer inline actions also support a deliberately narrow write subset: simple `$type = identifierOrString;` and `$channel = identifierOrString;` statements. The transformer rewrites those statements to `SetLexerType(result, "...")` and `SetLexerChannel(result, "...")`, which set bounded `LexerActionExecutionResult.TokenType` and `LexerActionExecutionResult.Channel` fields. Generated hooks do not mutate `Token` directly. `LexerEngine` applies accepted action-result mutations to the accepted token before lexer commands. Commands remain authoritative: `type(...)`, `channel(...)`, `skip`, and `more` keep their existing language-neutral behavior. Reads such as `$type` and `$channel` continue to read the passive `LexerActionExecutionContext`; writes do not retroactively change that read context inside the same action body.

Unsupported forms include `$text = ...`, `$mode = ...`, `$line = ...`, `$pos = ...`, `$type += ...`, `$channel ??= ...`, `$type++`, `$channel++`, writes inside lexer predicates, complex expression writes, `ref`/`out` writes, chained attributes, runtime-inline lexer execution, and a separate runtime lexer.

...
Supported forms include expression-bodied parser predicates, block-bodied parser predicates with `return`, and single- or multi-statement inline parser actions, for example:
...
Unsupported constructs are not promoted to executable generated hooks. Invalid C# inside a supported hook remains a Roslyn compilation error. This alignment does not add new executable ANTLR constructs and does not change `ParserEngine` or the default `Parse(...)` behavior.

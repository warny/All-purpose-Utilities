...
Unsupported constructs are not promoted to executable generated hooks. Visible unsupported embedded-code constructs in the generator AST now report warning `UP1029 EmbeddedCodeConstructNotExecutedByGenerator`, including lexer actions, lexer predicates, grammar actions, `@members`, `@init`, and `@after`. `@members` is not injected; provide callable members through a separate partial class instead. Invalid C# inside a supported parser predicate/action hook remains a Roslyn compilation error rather than a custom unsupported-construct diagnostic. This alignment does not add new executable ANTLR constructs and does not change `ParserEngine` or the default `Parse(...)` behavior.

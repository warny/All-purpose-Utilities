...
Generated C# can opt into a narrow lexer inline-action attribute rewrite set through `CSharpAntlrStyleParserEmbeddedCodeTransformer`. The supported read-only lexer action attributes are `$text`, `$type`, `$channel`, `$mode`, `$line`, and `$pos`; the default/no-op transformer preserves them unchanged. `$line` is rewritten to `GetRequiredLexerLine(context)` and reads `LexerActionExecutionContext.Line`. `$pos` is rewritten to `GetRequiredLexerPos(context)` and reads `LexerActionExecutionContext.Column`. Both values come from `SourceSpan` and identify the 1-based beginning of the accepted token/chunk; `$pos` is therefore this runtime's 1-based source column, not full ANTLR `charPositionInLine` compatibility.
